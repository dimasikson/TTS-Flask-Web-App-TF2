...
if __name__ == '__main__':
    ...
    logdir = os.path.join(hp.data, hp.logdir) + "-" + str(num)
    ...
    with sv.managed_session() as sess:
        while 1:
            for _ in tqdm(range(g.num_batch), total=g.num_batch, ncols=70, leave=False, unit='b'):
                gs, _, loss_mels, loss_bd1, loss_att = sess.run([g.global_step, g.train_op, g.loss_mels, g.loss_bd1, g.loss_att])

                log_loss_append['loss'].iloc[0] = loss_mels+loss_bd1+loss_att
                log_loss_append['loss_mels'].iloc[0] = loss_mels
                log_loss_append['loss_bd1'].iloc[0] = loss_bd1
                log_loss_append['loss_att'].iloc[0] = loss_att
                log_loss = log_loss.append(log_loss_append)

                # Write checkpoint files at every 1k steps
                if gs % 10 ==0:
                    print(str(round(loss_mels+loss_bd1+loss_att,4)) + " :  " + str(round(loss_mels,4)) + " - " + str(round(loss_bd1,4)) + " - " + str(round(loss_att,4)))
                ...
                if gs % g.num_batch == 0:
                    ...
                    if num==1:
                        ...
                        pd.DataFrame(alignments[0]).to_csv(os.path.join(hp.data, "outputs", "Alignments_" + timestamp + ".csv"))
                        pd.DataFrame(log_loss).to_csv(os.path.join(hp.data, "outputs", "orig_logloss_" + timestamp + ".csv"))
