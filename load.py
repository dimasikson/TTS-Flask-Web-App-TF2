...
def init(ttm_model_num=-1, ssrn_model_num=-1, mono_attention=True): 
	L = load_data(mode="synth", text_inputs="bruh")

	model_folder_name = str("finished_models")

	TTM_fnames = []
	SSRN_fnames = []
	for filename in os.listdir(model_folder_name):
	...
	model_TTM.load_weights(os.path.join(model_folder_name, TTM_fname))
	model_SSRN.load_weights(os.path.join(model_folder_name, SSRN_fname))

	return model_TTM, model_SSRN

def synthesize(text_inputs, ttm_model, ssrn_model):
    ...
    for j in range(hp.max_T):
        ...
        _Y, _Y_logits, _alignments, _max_attentions = ttm_model.predict([L, S, prev_max_attentions])
        Y[:, j, :] = _Y[:, j, :]
    ...
    Z, Z_logits = ssrn_model.predict(Y)
